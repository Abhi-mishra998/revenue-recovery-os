    ...
    if is_postgres():
        async with pg.with_user(owner_id) as conn:
            recs = await conn.fetch(
                f"SELECT {_COLS_SQL} FROM activities ORDER BY created_at DESC LIMIT $1", limit,
            )
        return rows_to_dicts(recs)
    cursor = _mongo.db().activities.find({"owner_id": owner_id}, {"_id": 0}).sort("created_at", -1).limit(limit)
    ...
    if is_postgres():
        async with pg.with_user(doc["owner_id"]) as conn:
            await conn.execute(
                ...
                doc["id"], doc["owner_id"], doc["client_id"],
