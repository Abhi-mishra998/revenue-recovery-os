    ...
    if is_postgres():
        async with pg.with_user(owner_id) as conn:
            recs = await conn.fetch(
                f"SELECT {_COLS_SQL} FROM followups ORDER BY created_at DESC LIMIT $1", limit,
            )
        return rows_to_dicts(recs)
    cursor = _mongo.db().followups.find({"owner_id": owner_id}, {"_id": 0}).sort("created_at", -1).limit(limit)
