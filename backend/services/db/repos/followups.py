...
_COLS = ("id::text", "owner_id::text", "proposal_id::text", "client_id::text",
         "generation_id::text", "channel", "draft_text",
         "context", "prompt_ref", "route_ref",
...
async def list_for_proposal(proposal_id: str, owner_id: str, limit: int = 100) -> list[dict]:
    ...
    cursor = _mongo.db().followups.find(
        {"owner_id": owner_id, "proposal_id": proposal_id}, {"_id": 0},
    ...
    if is_postgres():
        ...
        async with pg.with_user(owner_id) as conn:
            await conn.executemany(
                ...
                [(d["id"], d["owner_id"], d["proposal_id"], d["client_id"], d["generation_id"],
                  d["channel"], d["draft_text"], jsonb(d.get("context") or {}),
                  d.get("prompt_ref"), d.get("route_ref"),
                  d.get("confidence"), d.get("latency_ms"), d["created_at"])
                 for d in docs],
            )
        return
