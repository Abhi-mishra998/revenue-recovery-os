...
_COLS = ("id::text", "owner_id::text", "client_id::text",
         "channel_preference", "channel_counts",
         "typical_response_days::float8 AS typical_response_days",
         "response_rate::float8 AS response_rate",
         "last_outcomes", "recompute_count", "updated_at")
    ...
    if is_postgres():
        async with pg.with_user(owner_id) as conn:
            rec = await conn.fetchrow(
                f"SELECT {_COLS_SQL} FROM client_memory WHERE client_id = $1::uuid", client_id,
            )
    ...
    if is_postgres():
        async with pg.with_user(owner_id) as conn:
            res = await conn.execute(
                "DELETE FROM client_memory WHERE client_id = $1::uuid", client_id,
            )
