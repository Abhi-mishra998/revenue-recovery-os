...
_COLS = ("id::text", "seq", "actor_id", "actor_email", "action",
         "resource_type", "resource_id",
         "payload_hash", "prev_hash", "record_hash",
    ...
    if is_postgres():
        async with pg.bypass_user() as conn:
            await conn.execute(
                ...
                rec["id"], int(rec["seq"]), rec["actor_id"], rec["actor_email"],
                rec["action"], rec.get("resource_type"), rec.get("resource_id"),
                rec["payload_hash"], rec["prev_hash"], rec["record_hash"],
                rec["signature"], rec["public_key_fp"], rec["timestamp"],
            )
        return
