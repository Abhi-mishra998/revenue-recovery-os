...
async def list_for_owner(owner_id: str, limit: int = 10000) -> list[dict]:
    """Used by /me/data export. Tenant-scoped via RLS on PG."""
    if is_postgres():
    ...
    if is_postgres():
        async with pg.with_user(doc["owner_id"]) as conn:
            await conn.execute(
                ...
                doc["entity_type"],
                doc["entity_id"],
                jsonb(doc.get("prior_value")),
                jsonb(doc.get("new_value")),
                jsonb(doc.get("metadata") or {}),
                doc.get("source", "system"),
                doc["created_at"],
            )
        return
...
async def list_filtered(
    ...
    *,
    ...
    if is_postgres():
        ...
        if entity_id:
            clauses.append(f"entity_id = ${len(params) + 1}::uuid")
...
async def list_outcomes_for_client(
    owner_id: str, client_id: str, types: tuple[str, ...], limit: int = 10
) -> list[dict]:
    ...
    cursor = (
        _mongo.db()
        .events.find(
            {"owner_id": owner_id, "metadata.client_id": client_id, "event_type": {"$in": list(types)}},
            {"_id": 0},
        )
        .sort("created_at", -1)
        .limit(limit)
