...
from . import generate_proposal_followup
...
async def _run_one(
    ...
    if dry_run:
        ...
        ctx = _build_context(
            sender_name=fix.sender_name,
            recipient_contact=fix.recipient_contact,
            recipient_company=fix.recipient_company,
            industry=fix.industry,
            title=fix.title,
            value_inr=fix.value_inr,
            days_silent=fix.days_silent,
        )
    ...
    try:
        ...
        return RunRecord(
            fixture_id=fix.id,
            prompt_ref=template.ref,
            route_ref=result.get("route_ref", route_ref),
            ok=True,
            guardrail_ok=True,
...
def _print_table(rows: list[RunRecord]) -> None:
    ...
    for r in rows:
        ...
        print(
            f"| {r.fixture_id} | {r.prompt_ref} | {r.route_ref} | {ok} | {gd} | "
...
async def main() -> int:
    ...
    p.add_argument(
        "--prompts",
        nargs="+",
        default=[ACTIVE["proposal_followup"].ref],
    ...
    p.add_argument(
        "--fixtures", nargs="+", default=[f.id for f in FIXTURES], help="Fixture ids. Default: all."
    )
    ...
    for pref in args.prompts:
        if pref not in ALL:
            print(f"Unknown prompt ref: {pref}. Available: {list(ALL)}")
            return 2
