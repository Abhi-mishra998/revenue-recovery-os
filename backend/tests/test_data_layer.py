...
def client_with_proposal(s):
    c = s.post(
        f"{API}/clients",
        json={
            "company_name": f"DATA_{uuid.uuid4().hex[:6]}",
            "contact_name": "Data Test",
        },
        timeout=15,
    ).json()
    p = s.post(
        ...
        json={
            "client_id": c["id"],
            "title": "Data layer pilot",
            "value_inr": 200000,
            "stage": "sent",
            "sent_date": _iso_ago(2),
            "last_contact_date": _iso_ago(2),
        },
        timeout=15,
    ).json()
    ...
    def test_handles_missing_memory(self):
        f = extract_proposal_features(
            proposal={
                "stage": "sent",
                "value_inr": 1000,
                "last_contact_date": _iso_ago(1),
                "sent_date": _iso_ago(1),
            },
            memory=None,
        )
...
class TestProposalApiEnrichment:
    ...
    def test_stage_flip_did_not_stamp_outcome_at_initially(self):
        ...
        c = s2.post(
            f"{API}/clients",
            json={"company_name": f"X_{uuid.uuid4().hex[:5]}", "contact_name": "x"},
            timeout=15,
        ).json()
        p = s2.post(
            f"{API}/proposals",
...
class TestEvents:
    def test_proposal_created_emits_event(self):
        ...
        c = s2.post(
            f"{API}/clients",
            json={"company_name": f"E_{uuid.uuid4().hex[:5]}", "contact_name": "e"},
            timeout=15,
        ).json()
        p = s2.post(
            f"{API}/proposals",
    ...
    def test_invoice_payment_emits_event(self):
        ...
        inv = s2.post(
            ...
            json={
                "client_id": c["id"],
                "invoice_no": f"E-{uuid.uuid4().hex[:5]}",
                "amount_inr": 5000,
                "due_date": _iso_ago(2),
            },
            timeout=15,
...
class TestClientMemory:
    def test_memory_recomputes_after_activity(self):
        ...
        c = s2.post(
            f"{API}/clients",
            json={"company_name": f"M_{uuid.uuid4().hex[:5]}", "contact_name": "m"},
            timeout=15,
        ).json()
