...
def _register() -> dict:
    ...
    r = requests.post(f"{API}/auth/register",
                      json={"email": email, "password": "Pw1234!!XY", "name": "Edge"},
                      timeout=15)
...
class TestMalformedTokens:
    ...
    def test_wrong_signature(self):
        ...
        bad = jwt.encode({"sub": "anything", "email": "x@y", "tv": 0,
                          "exp": int(time.time()) + 60, "type": "access"},
                         "wrong-secret", algorithm=JWT_ALG)
    ...
    def test_wrong_algorithm(self):
        ...
        bad = jwt.encode({"sub": "x", "email": "x@y", "tv": 0,
                          "exp": int(time.time()) + 60, "type": "access"},
                         JWT_SECRET, algorithm="HS512")  # wrong alg
        r = requests.get(f"{API}/auth/me", headers=_hdrs(bad), timeout=15)
        assert r.status_code == 401
...
class TestTokenLifecycle:
    def test_expired_token_rejected(self):
        expired = jwt.encode({"sub": "any", "email": "x@y", "tv": 0,
    ...
    def test_logout_revokes_other_tokens_for_same_user(self):
        ...
        r = requests.post(f"{API}/auth/login",
                          json={"email": data["user"]["email"], "password": "Pw1234!!XY"},
                          timeout=15)
        token2 = r.json()["token"]
    ...
    def test_token_for_deleted_user(self):
        ...
        forged = jwt.encode({"sub": str(uuid.uuid4()), "email": "ghost@example.com",
                             "tv": 0, "exp": int(time.time()) + 3600,
                             "type": "access"},
                            JWT_SECRET, algorithm=JWT_ALG)
...
class TestRegisterEdges:
    def test_duplicate_email_rejected(self):
        ...
        r = requests.post(f"{API}/auth/register",
                          json={"email": email, "password": "Pw1234!!XY", "name": "Dup"},
                          timeout=15)
    ...
    def test_short_password_rejected(self):
        ...
        r = requests.post(f"{API}/auth/register",
                          json={"email": f"shortpw_{uuid.uuid4().hex[:6]}@x.com",
    ...
    def test_long_name_rejected(self):
        r = requests.post(f"{API}/auth/register",
                          json={"email": f"longname_{uuid.uuid4().hex[:6]}@x.com",
                                "password": "Pw1234!!XY", "name": "x" * 200},
                          timeout=15)
        assert r.status_code == 422
    ...
    def test_email_lowercased_on_register(self):
        ...
        r = requests.post(f"{API}/auth/register",
                          json={"email": mixed, "password": "Pw1234!!XY", "name": "M"},
                          timeout=15)
...
class TestLoginEdges:
    def test_wrong_password_401(self):
        data = _register()
        r = requests.post(f"{API}/auth/login",
                          json={"email": data["user"]["email"], "password": "WrongPw1!"},
                          timeout=15)
        assert r.status_code == 401

    def test_unknown_email_401(self):
        r = requests.post(f"{API}/auth/login",
                          json={"email": f"noone_{uuid.uuid4().hex[:6]}@x.com",
                                "password": "Pw1234!!XY"}, timeout=15)
        assert r.status_code == 401
    ...
    def test_login_case_insensitive(self):
        ...
        email = data["user"]["email"]
        r = requests.post(f"{API}/auth/login",
                          json={"email": email.upper(), "password": "Pw1234!!XY"},
                          timeout=15)
        assert r.status_code == 200
